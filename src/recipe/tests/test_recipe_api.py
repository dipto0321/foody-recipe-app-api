...
def image_upload_url(recipe_id):
    """Return image upload url for specific recipe"""
...
class RecipeFilterTests(TestCase):
    """Testing recipes filtering feature"""
    ...
    def test_filter_recipe_by_tags(self):
        """Get recipe by filtering tags"""
        recipe1 = sample_recipe(user=self.user, title='Mixed vegetable')
        recipe2 = sample_recipe(user=self.user, title='Fish curry with potato')
        recipe3 = sample_recipe(
            user=self.user, title='Boiled vegetable with cheese'
        )
        tag1 = sample_tag(user=self.user, name='Vegan')
        tag2 = sample_tag(user=self.user, name='Nonveg')
    ...
    def test_filter_Recipe_by_ingredients(self):
        """Get recipe by filtering ingredient"""
        recipe1 = sample_recipe(user=self.user, title='Vegetable salas')
        recipe2 = sample_recipe(
            user=self.user, title='Mutton curry with potato')
        recipe3 = sample_recipe(
            user=self.user, title='Boiled vegetable with eggs'
        )
